...
- built-in caller/IAB submission returns `awaiting_controller` after one durable send and exact URL capture; each later continuation performs one non-blocking observation and returns the same status again if Pro is unfinished. A recovered response is parsed only after the current request is correlated by exact visible request evidence, exact current envelope identity, or a reliable one-user-turn post-click increase. After a recovered non-terminal command is accepted and executed, caller mode returns `ready` before any next controller submission; another explicit continuation is required. Observation never presses `Answer now`, `Respond now`, `Stop`, or an equivalent interruption control
