...
- `phase: controller_decision_pending` with an already accepted `dispatch` may only need the next explicit continuation to execute the durable pending dispatch and register caller jobs. That path does not need a Browser binding because `resolveIabBrowser` is lazy. If the accepted dispatch has already been executed and no jobs remain pending, the next continuation may instead mint the next controller turn; browser failures should be diagnosed before retrying the controller send.
- `phase: controller_response_pending`, `controller.pendingTurns === 1`, missing runtime ownership, and `safeNextAction: recover_submitted_turn` mean the exact normally submitted request has durable write-ahead and message-count baselines for one read-only recovery observation. `continueCueLineRun` first reopens the exact conversation when necessary. A still-staged exact inline prompt or attachment, an absent current request, and idle Pro prove not-sent after a stable hydrated read even when a rebooted `0` baseline rises to the page's full historical count. Historical count uplift alone never proves that the current request was sent. Conversely, a long-conversation accessibility snapshot may supply a completed assistant envelope when the ordinary message DOM is virtualized to zero, but only from a ChatGPT-authored article and only when the exact conversation URL, selected Pro label, Pro response model slug, stopped-answering state, and full protocol/run/round/request identity all match. This exception never searches arbitrary page or user text. Other normally submitted turns remain `safeNextAction: observe`; `safeNextAction: reconcile` is reserved for ambiguous, manually submitted, or multiple pending turns.
...
### Misdirected submitted turns

If a submitted controller turn was sent into a different ChatGPT conversation, use the explicit misdirected recovery path instead of `--not-sent-confirmed`:
...
This path requires a Browser adapter and performs read-only observation only. It accepts recovery only when the orphan conversation contains the exact controller envelope for the same run, round, and request ID, while the bound conversation is idle, lacks the current request, and still ends at the prior accepted controller envelope. Success appends `controller_turn_misdirected_confirmed`, then the existing `controller_turn_not_sent_confirmed` recovery record, then abandons the old request without consuming the round. The next `continue` uses the same one-shot retry guard and `retry_of_request_id` chain as ordinary not-sent recovery. If the orphan response is still streaming, the command returns `pending` and writes no recovery events.
...
- omitted `maxRounds` is a durable unlimited contract for new runs; a positive explicit value remains a durable total-run cap, not a per-continuation allowance, and continuation cannot add, widen, shrink, or remove it
- `maxStagnantRounds` is durable across continuations; `controller_round_progress` compares identity-free structured controller output plus observed job evidence, and `stagnation_detected` is a fail-closed terminal failure that cannot be resumed
- when the built-in write-ahead contract or a request-correlated failure proves the exact sole pending request `definitely_not_sent`, CueLine records that attempt as abandoned without consuming the controller round and immediately returns; only a separate later continuation may create the one authorized retry
- when a restarted normally submitted turn has a durable user-message baseline, continuation may reclassify it from a fresh stable observation where the exact prompt remains staged, the current request is absent, and Pro is idle. Hydration may raise an unreliable persisted baseline to the full historical count; that uplift does not authorize stale assistant parsing. The append-only recovery permits one later retry with `retry_of_request_id` and never authorizes a retry of that retry
- a pending controller turn is reconciled read-only from the exact conversation before any new prompt is sent
- built-in caller/IAB submission returns `awaiting_controller` after one durable send and exact URL capture; each later continuation performs one non-blocking observation and returns the same status again if Pro is unfinished. A recovered response is parsed only after the current request is correlated by exact visible request evidence, exact current envelope identity, or the narrowly scoped accessibility-envelope exception above. After a recovered non-terminal command is accepted, caller mode returns `ready` before any next controller submission; when that response came through the count-degraded accessibility exception, a recovered `dispatch` also defers job registration until the next explicit continuation. Observation never presses `Answer now`, `Respond now`, `Stop`, or an equivalent interruption control
- inline reconciliation requires the page's last user message to equal the persisted prompt unless the completed assistant message itself carries the exact protocol/run/round/request envelope. That exact envelope may supersede a stale or missing virtualized user message only after the assistant-source, exact-conversation, stopped-answering, selected-Pro, and Pro-response-slug checks all pass
