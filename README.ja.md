...
- Send クリック完了後、CueLine は `controller_turn_submitted` を記録する前にページ上の確認証拠を待つようになりました。添付ファイルが composer に残る場合は `definitely_not_sent`、証拠なしで消えた場合は `possibly_sent` とし、二度目のクリックを行いません。未送信回復は安全境界で一時停止し、別の `continue` だけが一度送信できます。673/673 テストに合格しました。

詳細は [changelog](CHANGELOG.md#047---2026-07-20) またはバージョン指定の [v0.4.7 release](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.7) を参照してください。
...
フォールバックとして、[v0.4.7 リリース](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.7) のパッケージ済み tarball をインストールすることもできます。同じリリースに `.sha256` チェックサムも置いてあります。
