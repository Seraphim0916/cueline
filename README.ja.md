...
- 再起動後の recovery は、hydration による過去メッセージ数の増加を現在の request が送信済みである証拠として扱わなくなりました。CueLine は現在の request を正確に関連付けてから最後の assistant 応答を解析し、既存の応答を受理した後は `ready` で停止するため、次のラウンドには別の `continue` が必要です。681/681 テストに合格しました。

詳細は [changelog](CHANGELOG.md#048---2026-07-20) またはバージョン指定の [v0.4.8 release](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.8) を参照してください。
...
フォールバックとして、[v0.4.8 リリース](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.8) のパッケージ済み tarball をインストールすることもできます。同じリリースに `.sha256` チェックサムも置いてあります。
