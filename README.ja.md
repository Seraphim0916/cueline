...
- ステージ済みの composer 添付を再利用する retry は、添付に埋め込まれた元の `request_id` をコントローラから見える識別子として保持するようになりました。Pro の正しい応答が `CONTROL_ID_MISMATCH` で拒否されることはもうありません。旧来の二重識別子 retry で詰まった run は、永続レコードから読み取り専用で照合されます——再送なし、repair プロンプトなし、新しい round なし。667/667 テストに合格しました。

詳細は [changelog](CHANGELOG.md#045---2026-07-20) またはバージョン指定の [v0.4.5 release](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.5) を参照してください。
...
フォールバックとして、[v0.4.5 リリース](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.5) のパッケージ済み tarball をインストールすることもできます。同じリリースに `.sha256` チェックサムも置いてあります。
