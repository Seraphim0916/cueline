...
- `cueline mcp serve` を追加：CueLine の永続 run インターフェースを 7 つのツール（`cueline_start_run`、`cueline_continue_run`、`cueline_run_status`、`cueline_run_doctor`、`cueline_claim_caller_job`、`cueline_start_caller_job`、`cueline_list_runs`）として公開する Model Context Protocol（MCP）stdio サーバーです。どの MCP ホストも CLI を介さず run を直接駆動できます。JSON-RPC 2.0 トランスポートは手書き実装で——CueLine はランタイム npm 依存ゼロを維持——ツールの結果は API と同じ有界な証拠を返し、process 実行は同一呼び出しでの明示的な `allowProcessExecution: true` を引き続き要求します。
- 0.3.2 の修正（ハイドレーション後の新しい観測証拠から詰まった `submitted` ターンを `retryOfRequestId` 付きのちょうど 1 回のリトライで復旧）も含みます。MCP ハンドシェイク／拒否の統合テスト、実際の stdio スモーク、552/552 テストで検証しました。

詳細は [changelog](CHANGELOG.md#040---2026-07-18) またはバージョン指定の [v0.4.0 release](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.0) を参照してください。
...
フォールバックとして、[v0.4.0 リリース](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.0) のパッケージ済み tarball をインストールすることもできます。同じリリースに `.sha256` チェックサムも置いてあります。
