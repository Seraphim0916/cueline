...
- 再起動後にコントローラーターンが `submitted` として永続記録されたまま、実際には会話に届いておらず、run が来るはずのない返信を永遠に待ち続ける詰まりを修正しました。ハイドレーション完了後の新しい観測で、ユーザーメッセージ数が送信前ベースラインと同じ、当該ターンのメッセージが存在しない、かつコントローラーがアイドルであることが確認できた場合、CueLine はそのターンを未送達と安全に再分類し、`retryOfRequestId` を記録したリトライをちょうど 1 回だけ作成します——重複送信は決して行わず、保留ターンが 2 つになることもなく、未ハイドレーションのページで判定することもありません。`run doctor` はこの形状を `SUBMITTED_TURN_RECOVERY_REQUIRED` として安全な次の一手とともに表示し、無限の観測ループにはなりません。
- 実際に詰まった run を再現する赤/緑フィクスチャ、5 つのフェイルクローズド負例、保留ターンが 2 つにならないことを証明する再入ケース、543/543 テストで検証しました。

詳細は [changelog](CHANGELOG.md#032---2026-07-18) またはバージョン指定の [v0.3.2 release](https://github.com/Seraphim0916/cueline/releases/tag/v0.3.2) を参照してください。
...
フォールバックとして、[v0.3.2 リリース](https://github.com/Seraphim0916/cueline/releases/tag/v0.3.2) のパッケージ済み tarball をインストールすることもできます。同じリリースに `.sha256` チェックサムも置いてあります。
