...
- 過去応答の照合は、記録済みコマンドを受理した後に明示的に一時停止するようになりました。0.4.6 では同じ呼び出し内で次の round を生成し、ブラウザの送信経路を呼び出す可能性がありました。次の round は新しい独立した `continue` でのみ進行します。この不具合で汚染された round も、正式な未送信確認の経路で回復できます。669/669 テストに合格しました。

詳細は [changelog](CHANGELOG.md#046---2026-07-20) またはバージョン指定の [v0.4.6 release](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.6) を参照してください。
...
フォールバックとして、[v0.4.6 リリース](https://github.com/Seraphim0916/cueline/releases/tag/v0.4.6) のパッケージ済み tarball をインストールすることもできます。同じリリースに `.sha256` チェックサムも置いてあります。
