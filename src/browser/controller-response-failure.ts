...
export const CHATGPT_THINKING_FAILED_CODE = "CHATGPT_THINKING_FAILED" as const;
export const CHATGPT_THINKING_FAILED_MESSAGE = "Thinking failed" as const;

/** Stable identity for an exact read-only controller response failure. */
export function controllerResponseFailureEvidenceHash(
  evidence: BrowserSubmittedTurnEvidence,
): string {
  return commandHash({
    schema: "cueline/controller-response-failure-evidence/v1",
    ...
    request_message_scan_complete:
      evidence.requestMessageScanComplete ?? false,
    ...
    composer_send_button_enabled:
      evidence.composerSendButtonEnabled ?? null,
    response_failure: evidence.responseFailure ?? null,
  });
}
