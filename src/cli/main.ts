...
function usage(): string {
  return "usage: cueline <install|uninstall|doctor|routing|jobs|runs|protocol lint|run status|run status-at|run doctor|run watch|run handoff|run timeline|run verify|run reconcile|run takeover|run reconcile-runtime|run cancel|run stop|job cancel|api path|config path|help|version>";
}
...
function help(): string {
  return [
    ...
    "",
    "state effects:",
    "  Read-only: doctor, routing, jobs, runs, protocol lint, run status, run status-at, run doctor, run watch, run handoff, run timeline, run verify, api path, config path, help, version.",
