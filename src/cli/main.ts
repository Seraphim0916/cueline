...
function usage(): string {
  return "usage: cueline <install|uninstall|doctor|routing|jobs|protocol lint|run status|run reconcile|run takeover|run reconcile-runtime|run cancel|run stop|job cancel|api path|config path|help|version>";
}
...
function help(): string {
  return [
    ...
    "",
    "state effects:",
    "  Read-only: doctor, routing, jobs, protocol lint, run status, api path, config path, help, version.",
