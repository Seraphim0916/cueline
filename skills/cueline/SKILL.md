...
- The controller field is `runner`, never `runner_id`. Every wait/inspect `job_ids` entry must be copied exactly from the current observation; one unknown target rejects the whole command before partial action. Caller is the default executor. Caller `advise` is a coordination-only handoff; caller `work` requires claim, start, heartbeat, and fenced result proof. A controller `dispatch` alone never executes local work.
