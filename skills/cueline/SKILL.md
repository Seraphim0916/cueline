...
- Controller fields are exact per action: use job `task`, never `prompt`, and `runner`, never `runner_id`; do not attach fields from another action. `job_ids`, when present for wait/inspect, must be non-empty and unique. For a truncated evidence tail, use one exact `job_ids` entry, copy its non-null `evidence_window.next_offset` as `evidence_offset`, and copy `content_hash` as `evidence_hash`; never guess either value or rerun the job. Caller is the default executor. Caller `advise` is a coordination-only handoff; caller `work` requires claim, start, heartbeat, and fenced result proof. A controller `dispatch` alone never executes local work.
